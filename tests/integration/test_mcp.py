...
class TestGetBestNetwork:
    ...
    async def test_get_best_network_valid_input(self):
        ...
        with patch('src.mcp.tools.get_best_network.calculate_all_scores') as mock_scores, \
...
class TestCompareNetworks:
    ...
    async def test_compare_networks_invalid_network(self):
        with pytest.raises(ValidationError):
            CompareNetworksInput(
                networks=["ethereum", "invalid_network"],
                amount_usd=1000.0,
                asset="USDC"
            )
        ...
        with pytest.raises(ValidationError):
            CompareNetworksInput(
                networks=["ethereum", "polygon", "arbitrum", "base", 
                         "optimism", "avalanche", "bsc", "solana", "extra"],
                amount_usd=1000.0,
                asset="USDC"
            )
...
class TestEstimateCost:
    ...
    async def test_estimate_cost_all_valid_networks(self):
        valid_networks = ["ethereum", "solana", "polygon", "arbitrum", 
